#!/bin/bash
# Test 11: API List Agents
# Tests the GET /api/v2/pyro-detector/agents endpoint

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESULTS_DIR="$SCRIPT_DIR/../results"
LOG_FILE="$RESULTS_DIR/logs/test-11-api-list-agents.log"
RESULT_FILE="$RESULTS_DIR/test-11-api-list-agents.json"
...
log "=== Test 11: API List Agents ==="
log "Starting test at $(date)"

API_BASE="${API_BASE:-http://localhost:8080}"
API_URL="${API_BASE}/api/v2/pyro-detector/agents"
...
cat > "$RESULT_FILE" <<EOF
{
    "test_id": "test-11-api-list-agents",
    "test_name": "API List Agents",
