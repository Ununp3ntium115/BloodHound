#!/bin/bash
# Test 09: API Execute Detonator
# Tests the POST /api/v2/pyro-detector/detonators/{id}/execute endpoint

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RESULTS_DIR="$SCRIPT_DIR/../results"
LOG_FILE="$RESULTS_DIR/logs/test-09-api-execute-detonator.log"
RESULT_FILE="$RESULTS_DIR/test-09-api-execute-detonator.json"
...
log "=== Test 09: API Execute Detonator ==="
log "Starting test at $(date)"

API_BASE="${API_BASE:-http://localhost:8080}"
DETONATOR_ID="${TEST_DETONATOR_ID:-test-detonator-001}"
API_URL="${API_BASE}/api/v2/pyro-detector/detonators/${DETONATOR_ID}/execute"
...
cat > "$RESULT_FILE" <<EOF
{
    "test_id": "test-09-api-execute-detonator",
    "test_name": "API Execute Detonator",
